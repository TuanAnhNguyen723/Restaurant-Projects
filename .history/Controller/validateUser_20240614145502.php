...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $row['Pass'])) {
            ...
            // Mật khẩu không đúng
            echo "<script>alert('Sai mật khẩu'); window.location='../index.php?error=invalid_credentials';</script>";
        }
    } else {
        // Tài khoản không tồn tại
        echo "<script>alert('Tài khoản không tồn tại'); window.location='../index.php';</script>";
    }
