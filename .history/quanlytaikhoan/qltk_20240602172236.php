  ...
  <body>
    ...
    if ($result->num_rows > 0) {
      ...
      while ($row = $result->fetch_assoc()) {
          ...
          echo "<td>" . $row["Role"] . "</td>";
          echo "<td>
                  <button class='btn active-btn' onclick='ActiveFn(" . $row["ID"] . ")'>" . ($row["Active"] == "block" ? "Không kích hoạt" : "Đang kích hoạt") . "</button>
                </td>";
          echo "</tr>";
      }
  } else {
    ...
    .then(data => {
        if (data.success) {
            ...
            if (button.innerText === 'Không kích hoạt') {
                button.style.backgroundColor = '#4caf50 !important'; // Thay đổi màu nền của nút khi chữ là "Không kích hoạt"
            } else {
                button.style.backgroundColor = '#f44336 !important'; // Đặt lại màu nền mặc định
            }
