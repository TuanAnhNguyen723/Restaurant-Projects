...
document.addEventListener('DOMContentLoaded', function() {
  var showCodesBtn = document.getElementById('showDiscountCodes');
  var discountSelect = document.getElementById('discountCode');

  showCodesBtn.addEventListener('click', function() {
    var xhr = new XMLHttpRequest();
    xhr.open('GET', 'getDiscountCodes.php', true); // Đổi đường dẫn này thành script PHP để lấy danh sách Mã giảm giá từ cơ sở dữ liệu
    xhr.onload = function() {
      if (xhr.status === 200) {
        var response = JSON.parse(xhr.responseText);
        if (response.success) {
          // Xóa các option hiện có trong select trước khi thêm mới
          discountSelect.innerHTML = '<option value="">Chọn Mã giảm giá</option>';

          // Thêm các Mã giảm giá vào dropdown
          response.discountCodes.forEach(function(code) {
            var option = document.createElement('option');
            option.value = code.MaGiam; // Thay 'MaGiam' bằng tên cột trong cơ sở dữ liệu của bạn
            option.textContent = code.MaGiam; // Thay 'MaGiam' bằng tên cột trong cơ sở dữ liệu của bạn
            discountSelect.appendChild(option);
          });
        } else {
          alert(response.message || 'Không thể lấy danh sách Mã giảm giá.');
        }
      } else {
