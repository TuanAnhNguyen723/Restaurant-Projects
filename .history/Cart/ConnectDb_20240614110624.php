...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (isset($_POST['discountInput'])) {
    $code = $_POST['discountInput'];

    // Truy vấn lấy thông tin giảm giá từ database
    $sql = "SELECT TiLe FROM discount WHERE MaGiam = '$code'";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $discount = $row["TiLe"];
        echo $discount; // Trả về tỉ lệ giảm giá
    } else {
        echo "0"; // Nếu không có mã giảm giá hợp lệ, trả về 0
    }
}
