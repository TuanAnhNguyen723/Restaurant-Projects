          ...
          <div class="box-dashboard">
            ...
            <p class="title-row">Nhập mã giảm giá</p>
                <div style="display: flex">
                    <select class="discount" id="discountCode">
                      <!-- Option mặc định để chọn tất cả -->
                      <option value="">Chọn tất cả</option>
                    </select>
                    <button class="btn-apdung">Áp dụng</button>
                    <button id="showAllCodes">Hiển thị tất cả Mã giảm giá</button>
                </div>   
            </div>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    var showAllBtn = document.getElementById('showAllCodes');
    var discountSelect = document.getElementById('discountCode');

    showAllBtn.addEventListener('click', function() {
      // Xóa các option hiện có trong select trước khi thêm mới
      discountSelect.innerHTML = '<option value="">Chọn tất cả</option>';

      // Lấy tất cả các mã giảm giá từ bảng
      var discountCodes = document.querySelectorAll('table tbody tr');
      var uniqueCodes = new Set(); // Dùng Set để lưu trữ mã giảm giá duy nhất

      discountCodes.forEach(function(row) {
        var discountCode = row.cells[5].innerText.trim(); // Lấy giá trị từ cột MaGiam (thay bằng chỉ số cột thực tế trong trường hợp của bạn)
        uniqueCodes.add(discountCode); // Thêm vào Set để loại bỏ các mã trùng lặp
      });
