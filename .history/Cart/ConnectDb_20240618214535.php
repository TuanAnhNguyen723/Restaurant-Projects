...
// Truy vấn để lấy danh sách Mã giảm giá
$sql = "SELECT MaGiam FROM discount"; // Thay 'DiscountCodes' bằng tên bảng trong cơ sở dữ liệu của bạn
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
  ...
  $response['discountCodes'] = $discountCodes;
} else {
  $response['success'] = false;
  $response['message'] = 'Không có Mã giảm giá nào được tìm thấy.';
}
