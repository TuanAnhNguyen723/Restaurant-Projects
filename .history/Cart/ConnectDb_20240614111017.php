...
// Truy vấn cơ sở dữ liệu để lấy tỉ lệ giảm giá tương ứng
$sql = "SELECT TiLe FROM discount WHERE MaGiam = '$discountCode'";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    $row = $result->fetch_assoc();
    $discountRate = $row['TiLe'];
    echo $discountRate;
} else {
    echo "0"; // Trả về 0 nếu không tìm thấy mã giảm giá
}
